const { connect, keyStores, KeyPair, utils, transactions } = require('near-api-js');
 ...
 */
async function sendTransaction(account, receiverId, amount) {
  ...
  try {
    ...
    const result = await account.signAndSendTransaction({
      ...
      waitUntil: 'FINAL'  // Wait for final confirmation
    });
 ...
 */
async function sendTransactionsParallel(account, receiverId, amount, numTxs, rate) {
  const delayMs = rate > 0 ? 1000 / rate : 0;
  const amountYocto = utils.format.parseNearAmount(amount);

  console.log('\nSending transactions in parallel with waitUntil: FINAL...');

  // Send all transactions with waitUntil: FINAL
  const txPromises = [];
  for (let i = 0; i < numTxs; i++) {
    ...
    const txPromise = (async () => {
      try {
        ...
        console.log(`  [${i + 1}/${numTxs}] ✓ Finalized in ${latency}ms`);
        ...
        const finalTime = Date.now();
        console.log(`  [${i + 1}/${numTxs}] ✗ Failed: ${error.message}`);
        return {
          txId: null,
          sendTime,
          finalTime,
  ...
  console.log('\nWaiting for all transactions to finalize...');
    ...
    for (let i = 0; i < config.numTxs; i++) {
      ...
      try {
        const result = await retry(
          () => sendTransaction(account, config.receiverId, config.amount),
          3,
          2000
        );
